...
public class ServiceFinderServer {
        ...
        public static void start() throws Exception {
                ...
                ServiceValidator validator = new ServiceValidatorImpl();
                ...
                server.createContext("/api/categories", exchange -> {
                        ...
                        var categories = repository.findAll().stream()
                                        ...
                                        .map(ServiceRecord::category)
                                        .filter(Objects::nonNull)
                                        .map(String::trim)
                                        .filter(category -> !category.isEmpty())
                                        .collect(Collectors.toMap(
                                                        category -> category.toLowerCase(java.util.Locale.ROOT),
                                                        ServiceFinderServer::formatCategory,
                                                        (first, ignored) -> first))
                                        .values().stream()
                                        .sorted(String.CASE_INSENSITIVE_ORDER)
                                        .toList();

                        byte[] responseBytes = new ObjectMapper()
                                        .writeValueAsBytes(categories);
                ...
                server.createContext("/api/services", exchange -> {
                        ...
                        if (query != null) {
                                ...
                                for (String parameter : query.split("&")) { // Split the query string into individual
                                        ...
                                        String[] parts = parameter.split("=", 2); // Split the parameter into name and
                                                                                  // value, limit to 2 parts
                                        if (parts.length != 2) {
                                                continue;
                                        }
                ...
                server.createContext("/", exchange -> {
                        ...
                        var resource = ServiceFinderServer.class
                                        .getClassLoader()
